...
const SHOPIFY_STORE_PERMANENT_DOMAIN = 'enasa-spark-boost-7jpuc.myshopify.com';
const SHOPIFY_STOREFRONT_URL = `https://${SHOPIFY_STORE_PERMANENT_DOMAIN}/api/${SHOPIFY_API_VERSION}/graphql.json`;
const SHOPIFY_STOREFRONT_TOKEN = 'b4a96d7dcda12e562247e73518693488';
...
export async function createStorefrontCheckout(items: any[]): Promise<string> {
  try {
    ...
    if (cartData.data.cartCreate.userErrors.length > 0) {
      const errorMessages = cartData.data.cartCreate.userErrors.map((e: any) => e.message).join(', ');
